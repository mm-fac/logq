...
func run(args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	case "head":
		return runHead(cmdArgs, *format, *strict, stdin, stdout, stderr)
	case "tail":
...
commands:
  ...
  head      print the first N records (default 10; -n N)
...
func runHead(args []string, format string, strict bool, stdin io.Reader, stdout, stderr io.Writer) int {
	// Re-register the common flags so they may also appear after the command.
	fs := flag.NewFlagSet("head", flag.ContinueOnError)
	fs.SetOutput(stderr)
	fs.StringVar(&format, "format", format, "output format: table|json|logfmt")
	fs.BoolVar(&strict, "strict", strict, "treat malformed input lines as a fatal error")
	n := fs.Int("n", 10, "number of leading records to print")
	if err := fs.Parse(args); err != nil {
		return 2
	}
	if *n <= 0 {
		fmt.Fprintf(stderr, "logq: head -n must be a positive integer (got %d)\n", *n)
		return 2
	}
